...
function Section({ title, icon: Icon, gradient, children, defaultOpen = true }) {
  ...
  return (
    <div className="bg-white/5 border border-white/8 rounded-xl overflow-hidden">
      <button type="button" onClick={() => setOpen(!open)} className="w-full flex items-center justify-between p-4 hover:bg-white/6 transition-colors">
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg bg-linear-to-r ${gradient} flex items-center justify-center`}>
  ...
  return (
    <div>
      ...
      <input
        ...
        className={`w-full px-3 py-2 rounded-lg border text-sm text-white focus:outline-hidden focus:border-primary focus:ring-1 focus:ring-primary/20 placeholder:text-slate-600 ${
          undetected ? 'border-amber-500/30 bg-amber-500/10' : 'border-white/8 bg-white/5'
        }`}
      />
    </div>
  )
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full px-3 py-2 rounded-lg border border-white/8 bg-white/5 text-white text-sm focus:outline-hidden focus:border-primary placeholder:text-slate-600"
      />
    </div>
  )
}
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2">
      <div className="col-span-2">
        ...
        <input type="text" value={product.name} onChange={(e) => updateProduct(index, 'name', e.target.value)} placeholder="Nome do produto" className="w-full px-3 py-2 rounded-lg border border-white/8 bg-white/5 text-white text-sm focus:outline-hidden focus:border-primary placeholder:text-slate-600" />
      </div>
      ...
      <div>
        ...
        <input type="text" value={product.shopeeItemId || ''} onChange={(e) => updateProduct(index, 'shopeeItemId', e.target.value)} className="w-full px-3 py-2 rounded-lg border border-white/8 bg-white/5 text-white text-sm focus:outline-hidden focus:border-primary placeholder:text-slate-600" />
      </div>
    </div>
  )
}

const emptyProduct = { name: '', canonicalName: '', price: 0, productClicks: 0, clickRate: 0, orders: 0, itemsSold: 0, orderClickRate: 0, addToCart: 0, revenue: 0, shopeeItemId: '' }
    ...
    <div className="space-y-4">
      ...
      <Section title="Marketing / Promoção" icon={Megaphone} gradient="from-amber-400 to-orange-500">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Usuarios unicos que resgataram</p>
          </div>
          <Field label="Rodada de Moedas" name="auctionRounds" value={data.auctionRounds} onChange={handleChange} format="integer" undetected={undetectedFields.includes('auctionRounds')} />
        </div>
      </Section>

      {/* Funil de Tráfego + Fontes */}
      <Section title="Funil de Tráfego" icon={MousePointerClick} gradient="from-cyan-400 to-blue-500" defaultOpen={false}>
        <div className="mb-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <Field label="Taxa de Cliques (%)" name="funnelClickRate" value={data.funnelClickRate} onChange={handleChange} format="percent" undetected={undetectedFields.includes('funnelClickRate')} />
            <Field label="Cliques de Produto" name="funnelProductClicks" value={data.funnelProductClicks} onChange={handleChange} format="integer" undetected={undetectedFields.includes('funnelProductClicks')} />
            ...
            <Field label="Pedido Confirmado" name="funnelOrders" value={data.funnelOrders} onChange={handleChange} format="integer" undetected={undetectedFields.includes('funnelOrders')} />
        ...
        {/* Fontes de Trafego - dentro do mesmo card */}
        <div className="mt-4 pt-4 border-t border-white/8">
          <h4 className="text-sm font-semibold text-slate-300 mb-3">Fontes de Tráfego ({(data.trafficSources || []).length})</h4>
          <div className="space-y-2">
            {(data.trafficSources || []).map((ts, index) => (
              <div key={index} className="bg-white/3 rounded-lg p-3 border border-white/4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-slate-300">Fonte {index + 1}</span>
                  <button type="button" onClick={() => {
                    const updated = (data.trafficSources || []).filter((_, i) => i !== index)
                    onChange({ ...data, trafficSources: updated })
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input type="text" value={ts.source || ''} onChange={(e) => {
                      const updated = [...(data.trafficSources || [])]
                      updated[index] = { ...updated[index], source: e.target.value }
                      onChange({ ...data, trafficSources: updated })
                    }} placeholder="Ex: Painel Ao Vivo"
                    className="w-full px-3 py-2 rounded-lg border border-white/8 bg-white/5 text-white text-sm focus:outline-hidden focus:border-primary placeholder:text-slate-600" />
                  </div>
                  <ProductField label="Relação Tráfego (%)" value={ts.trafficRate}
                    onChange={(v) => {
                      const updated = [...(data.trafficSources || [])]
                      updated[index] = { ...updated[index], trafficRate: typeof v === 'number' ? v : parseFloat(v) || 0 }
                      onChange({ ...data, trafficSources: updated })
                    }} format="integer" />
                  <ProductField label="Visualizações" value={ts.pageViews}
                    onChange={(v) => {
                      const updated = [...(data.trafficSources || [])]
                      updated[index] = { ...updated[index], pageViews: typeof v === 'number' ? v : parseInt(v) || 0 }
                      onChange({ ...data, trafficSources: updated })
                    }} format="integer" />
                </div>
              </div>
            ...
            <button type="button" onClick={() => {
              onChange({ ...data, trafficSources: [...(data.trafficSources || []), { source: '', trafficRate: 0, pageViews: 0 }] })
            }} className="w-full flex items-center justify-center gap-2 py-2.5 border-2 border-dashed border-white/12 rounded-lg text-sm font-medium text-slate-400 hover:border-primary hover:text-primary transition-colors">
              <Plus className="w-4 h-4" /> Adicionar Fonte de Tráfego
            </button>
          </div>
        </div>
      </Section>
      ...
      <Section title={`Produtos Apresentados (${products.length})`} icon={ShoppingCart} gradient="from-indigo-400 to-violet-500">
        <div className="space-y-3">
          ...
          <button type="button" onClick={addProduct} className="w-full flex items-center justify-center gap-2 py-2.5 border-2 border-dashed border-white/12 rounded-lg text-sm font-medium text-slate-400 hover:border-primary hover:text-primary transition-colors">
